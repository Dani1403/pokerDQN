...
class GraphicViewer(viewer.PokerViewer):
    ...
    def _run_flask(self) -> None:
        print("[FLASK] Starting _run_flask...")

        from gevent import monkey
        monkey.patch_all()
        print("[FLASK] Gevent monkey patched.")

        import flask
        import flask_socketio
        import markupsafe

        print("[FLASK] Flask and SocketIO imported.")
        ...
        print(f"[FLASK] Template path: {templates_path}")
        ...
        try:
            ...
            print("[FLASK ERROR] While initializing Flask app:", e)
            raise
        ...
        def index():  # type: ignore
            ...
            try:
                ...
                print("[FLASK ERROR] While generating SVG:", e)
                raise
            ...
            while True:
                if conn.poll():
                    ...
                    if message["content"] == "close":
                        ...
                        print("[GRAPHIC] Emitting config:", config)

                        socketio.emit("config", config)
                socketio.sleep(0.0001)
            socket.close()

        print("[FLASK] Starting listener thread...")
        socketio.start_background_task(listener)

        try:
            print(f"[FLASK] Starting server at {self.port}")
            socketio.run(app, port=self.port)
        except Exception as e:
            print("[FLASK ERROR] While starting server:", e)
            raise
    ...
    def render(self, config: viewer.RenderConfig, sleep: float = 0) -> None:
        ...
        ... }
        """
        print("[GRAPHIC] Sending config to socket :", config)
