...
class Evaluator(object):
    ...
    def evaluate(
        ...
        return minimum
        """
        # Build an iterator of card-combos instead of lists
        if self.mandatory_hole_cards:
            hole_card_combs = itertools.combinations(hole_cards, self.mandatory_hole_cards)
            num_comm = self.cards_for_hand - self.mandatory_hole_cards
            if num_comm:
                comm_card_combs = itertools.combinations(community_cards, num_comm)
                iterator = (sum(card_comb, ()) for card_comb in itertools.product(hole_card_combs, comm_card_combs))
            else:
                iterator = hole_card_combs
        else:
            iterator = itertools.combinations(hole_cards + community_cards, self.cards_for_hand)

        best = self.table.max_rank
        # Lower is better: 1 is best possible early exit if hit
        for combo in iterator:
            score = self.table.lookup(list(combo))
            if score < best:
                if score == 1:       # perfect hand rank, nothing beats it
                    return 1
                best = score
        return best
...
class LookupTable:
    ...
    def lookup(self, cards: List[card.Card]) -> int:
        ...
        """
        # Check flush by ANDing suits without Python dunders
        flush_mask = 0xF000
        rankbits_or = 0
        for c in cards:
            ci = c._int
            flush_mask &= ci
            rankbits_or |= ci
        if flush_mask:  # all same suit
            prime = _prime_product_from_rank_bits(rankbits_or >> 16)
            return self.suited_lookup[prime]

        # otherwise unsuited lookup
        prime = _prime_product_from_hand(cards)
        return self.unsuited_lookup[prime]
        ...
        def add_to_dict(
            ...
            for card_ranks in multiple_combinations:
                ...
                if num_kickers:
                    ...
                    # )
                    kicker_combinations = itertools.combinations(
                        kickers, num_kickers)
