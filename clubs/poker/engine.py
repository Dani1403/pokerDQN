...
class Dealer:
    ...
    def render(self, mode: str = "human", sleep: float = 0, **kwargs: Any) -> None:
        ...
        config = self._render_config()
        #print("[ENGINE] sending config:", config)
    ...
    def _eval_round(self) -> List[int]:
        ...
        if not hasattr(self, "_hands_buf"):
            self._hands_buf = [[0, 0, 0] for _ in range(self.num_players)]
        hands = self._hands_buf
        for i, strength in enumerate(hand_strengths):
            hands[i][0] = i
            hands[i][1] = strength
            hands[i][2] = self.pot_commits[i]
        hands.sort(key=operator.itemgetter(1, 2))
